...
import { Badge, Button, Card, EmptyState, ErrorState, Select, Spinner, useToast } from '@/components/ui';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">Statements by account + date range, exports, and a ledger-wide reconciliation check.</p>
      </div>
        ...
        <div className="flex flex-wrap items-end gap-4">
          ...
          <label className="flex flex-col gap-1 text-sm text-slate-600">
            ...
            <input
              ...
              className="min-h-[44px] rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20"
            />
          </label>
          <label className="flex flex-col gap-1 text-sm text-slate-600">
      ...
      <Card className="p-5 print:border-0 print:shadow-none">
        <div className="flex flex-wrap items-baseline justify-between gap-2 border-b border-slate-100 pb-3">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">{account?.name ?? 'Statement'}</h2>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200 text-left text-xs uppercase tracking-wide text-slate-400">
            ...
            <th className="py-2 pr-3 text-right font-medium">Running</th>
        ...
        <tbody className="divide-y divide-slate-100">
          {statement.items.map((e) => (
            <tr key={e.seq}>
              <td className="whitespace-nowrap py-2 pr-3 tabular-nums text-slate-500">{formatDateTime(e.postedAt)}</td>
              <td className="py-2 pr-3 capitalize text-slate-700">{e.type}</td>
              <td className={`whitespace-nowrap py-2 pr-3 text-right tabular-nums font-medium ${e.direction === 'debit' ? 'text-emerald-700' : 'text-rose-700'}`}>
                {formatAmount(e.amount, currency)}
              </td>
              <td className="whitespace-nowrap py-2 pr-3 text-right tabular-nums text-slate-900">{formatAmount(e.runningBalance, currency)}</td>
              <td className="max-w-[22rem] truncate py-2 text-slate-500" title={e.explanation}>
                {e.explanation}
              </td>
            </tr>
          ))}
        ...
        {query.isLoading ? (
          ...
          <div className="space-y-3">
            <div className="flex flex-wrap gap-3 text-sm">
              ...
              <span className="text-slate-600">Checked {report.checked.transactions} transactions · {report.checked.accounts} accounts</span>
            ...
            {report.issues.length === 0 ? (
              ...
              <ul className="divide-y divide-slate-100 rounded-lg border border-rose-200">
                {report.issues.map((issue, i) => (
                  <li key={i} className="px-3 py-2 text-sm text-rose-700">
                    <span className="font-mono text-xs">{issue.type}</span> — {issue.message}
                  </li>
                ))}
              </ul>
            )}
